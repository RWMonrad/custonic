...
async function main() {
  ...
  try {
    ...
    const { data: aliceOrgId } = await alice.rpc('create_org_and_make_owner', { org_name: 'Alice Download Test' })
    ...
    const { data: bobUrl, error: bobError } = await bob.storage
      .from('contracts')
      .createSignedUrl(`contracts/${aliceOrgId}/${testContractId}/test.pdf`, 60)
    ...
    const { data: aliceUrl, error: aliceError } = await alice.storage
      .from('contracts')
      .createSignedUrl(`contracts/${aliceOrgId}/${testContractId}/test.pdf`, 60)

    if (aliceError) {
      console.log('⚠️ Alice access to non-existent contract failed (expected):', aliceError.message)
    } else {
      console.log('❌ UNEXPECTED: Alice got signed URL for non-existent contract')
    }

    console.log('\n✅ M4 Download Security Test Completed!')
    console.log('\n📋 Expected Results:')
    console.log('- Bob access to Alice contract: BLOCKED by Storage RLS')
    console.log('- Alice access to non-existent contract: BLOCKED (file not found)')
