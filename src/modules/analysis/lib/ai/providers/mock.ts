...
export class MockProvider implements AIProvider {
  ...
  private createSeed(chunkText: string, chunkIndex: number): number {
    const str = `${chunkText}-${chunkIndex}`;
    let hash = 0;
  ...
  async analyzeChunk(
    ...
    // Simulate processing delay only in development with MOCK_DELAY enabled
    if (
      process.env.AI_PROVIDER === "mock" &&
      process.env.MOCK_DELAY === "true"
    ) {
      const delay = Math.max(50, 150 - (chunkIndex / totalChunks) * 100);
  ...
  async synthesizeFindings(
    ...
    const finalFindings = this.generateFinalFindings(
      fullText,
      candidateFindings,
    );
  ...
  private generateMockFindings(chunkText: string, chunkIndex: number) {
    ...
    const baseSeed = this.createSeed(chunkText, chunkIndex);
    ...
    for (const [keyword, risk] of Object.entries(riskKeywords)) {
      if (text.includes(keyword) && findingIndex < 3) {
        ...
        const findingSeed = baseSeed + findingIndex;
        ...
        findings.push({
          ...
          confidence:
            risk.confidence +
            (this.seededRandom(findingSeed) * 0.2 - 0.1) +
            chunkIndex * 0.01,
          ...
          excerpt,
          confidence_boost:
            this.seededRandom(findingSeed + 1000) * 0.2 -
            0.1 +
            chunkIndex * 0.005,
        });
        findingIndex++;
  ...
  private generateFinalFindings(
    fullText: string,
    candidateFindings: CandidateFindings,
  ) {
    // Extract all candidate findings from chunks and track their origin
    const allFindings = candidateFindings.flatMap((chunk, chunkIndex) =>
      chunk.candidate_findings.map((finding) => ({
        ...finding,
        chunk_index: chunkIndex,
      })),
    );
    ...
    const topFindings = allFindings
      .sort(
        (a, b) =>
          b.confidence +
          b.confidence_boost -
          (a.confidence + a.confidence_boost),
      )
      .slice(0, 5);
    ...
    return topFindings.map((finding, index) => ({
      ...
      confidence: Math.min(
        1,
        Math.max(0, finding.confidence + finding.confidence_boost),
      ),
  ...
  private generateMockSummary(fullText: string): string {
    ...
    summary +=
      "that require careful review to ensure adequate risk protection.";
