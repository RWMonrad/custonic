...
export function FindingCard({ finding, isExpanded, onToggleExpanded }: FindingCardProps) {
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full border ${getSeverityColor(finding.severity)}`}>
                {finding.severity}
              </span>
              <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                {finding.category.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
              </span>
            ...
            <div className="flex items-center justify-between">
              ...
              {finding.citations.length > 0 && (
                ...
                >
                  {isExpanded ? 'Hide' : 'Show'} Evidence ({finding.citations.length})
                </button>
              )}
            </div>
          </div>
          ...
          <div className="p-4">
                ...
                <div key={index} className="bg-white border rounded p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className="text-xs text-primary hover:text-primary/80 underline"
                    >
                      {copiedExcerpt === `${finding.id}-${index}` ? 'Copied!' : 'Copy'}
                    </button>
                  </div>
            ...
            <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
              <p className="text-xs text-yellow-800">
                <strong>Note:</strong> These excerpts are provided as evidence for the identified risk. 
                Please review the full contract context for complete understanding.
              </p>
            </div>
          </div>
        </div>
