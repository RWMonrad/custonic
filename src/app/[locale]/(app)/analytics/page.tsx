...
export default function AnalyticsPage() {
  ...
  const columns = [
    {
      ...
      render: (value: string) => (
        <span className={`
          ...
          ${value === 'Negotiation' ? 'bg-primary/10 text-primary' : ''}
          ${value === 'Termination' ? 'bg-success/10 text-success' : ''}
          ${value === 'Renegotiation' ? 'bg-warning/10 text-warning' : ''}
          ${value === 'Compliance' ? 'bg-info/10 text-info' : ''}
          ${value === 'Optimization' ? 'bg-muted text-muted-foreground' : ''}
        `}>
          {value}
        </span>
      ),
    ...
    },
    {
      ...
      title: 'Realized',
      sortable: true,
      render: (value: string, row: SavingsData) => (
        <div className="flex items-center space-x-2">
          <span className="font-medium text-foreground">{value}</span>
          {row.status === 'realized' && row.realizedSavings !== row.identifiedSavings && (
            <span className="text-xs text-muted-foreground">
              ({Math.round((parseFloat(row.realizedSavings.replace(/[$,]/g, '')) / parseFloat(row.identifiedSavings.replace(/[$,]/g, ''))) * 100)}%)
            </span>
          )}
        </div>
      ),
    },
    {
      ...
      render: (value: string) => (
        <span className={`
          inline-block px-2 py-1 text-xs rounded-full
          ${value === 'realized' ? 'bg-success/10 text-success' : ''}
          ${value === 'in_progress' ? 'bg-warning/10 text-warning' : ''}
          ${value === 'identified' ? 'bg-muted text-muted-foreground' : ''}
        `}>
          {value.replace('_', ' ')}
        </span>
      ),
  ...
  const totalIdentified = savingsData.reduce((sum, item) => 
    sum + parseFloat(item.identifiedSavings.replace(/[$,]/g, '')), 0
  );
  const totalRealized = savingsData.reduce((sum, item) => 
    sum + parseFloat(item.realizedSavings.replace(/[$,]/g, '')), 0
  );
    ...
    <AppLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Savings & Analytics</h1>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Total Identified</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Realization Rate</span>
                    <span className="font-medium text-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {['Negotiation', 'Termination', 'Renegotiation', 'Compliance', 'Optimization'].map((category) => {
                  const categorySavings = savingsData.filter(s => s.category === category);
                  const total = categorySavings.reduce((sum, s) => 
                    sum + parseFloat(s.realizedSavings.replace(/[$,]/g, '')), 0
                  );
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {[
                  { status: 'realized', label: 'Realized', color: 'text-success' },
                  { status: 'in_progress', label: 'In Progress', color: 'text-warning' },
                  { status: 'identified', label: 'Identified', color: 'text-muted-foreground' },
                ].map(({ status, label, color }) => {
                  const count = savingsData.filter(s => s.status === status).length;
                  const percentage = Math.round((count / savingsData.length) * 100);
